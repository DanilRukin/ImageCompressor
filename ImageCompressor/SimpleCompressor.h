...
class SimpleCompressor : public BaseCompressor
{	
public:
	unsigned short* Compress(unsigned int* pixels, int width, int height) override
	{
		...
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				...
				bgra = b | g | r | alfa;
				compressedPixels[i * width + j] = bgra;
	...
	unsigned int* Decompress(unsigned short* compressedPixels, int width, int height) override
	{
		...
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				compressedPixel = compressedPixels[i * width + j];
				//
				...
				pixels[i * width + j] = restoredPixel;
			}
		}
		return pixels;
	}
